...
import { generateAccessToken } from '../utils/createJwtToken';
...
export const checkJwt = (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    jwtPayload = jwt.verify(token, process.env.JWT_ACCESS_SECRET as string) as { [key: string]: any };
  ...
  try {
    ...
    const newToken = generateAccessToken(jwtPayload as JwtPayload);
